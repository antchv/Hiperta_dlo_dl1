	...
	std::for_each(std::execution::par_unseq, std::begin(vecIndex), std::end(vecIndex),
        [=](int idx) {
			...
			for(size_t j=begin; j < end; j++){
				integ[idx] += signal[evtidx * (nbSlice * nbPixel) + j*nbPixel + pixelidx];
			}
...
void evaluateIntegration(size_t nbEvent){
	...
	micro_benchmarkAutoNsPrint("evaluateCalibration 2d tranform", fullNbElement, compute_integration, tabSignal, vecIdx, NB_SLICE, NB_PIXEL, nbSliceWindow, nbSliceBeforePeak, tabinteg);
